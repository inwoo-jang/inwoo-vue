...
}`},note:`취소를 누르면 reject 되어 catch 로 간다. catch 를 빼먹으면 콘솔에 처리되지 않은 Promise 경고가 뜬다.`}],checklist:[`UI 라이브러리를 쓰면 무엇이 절감되는지 세 가지로 말할 수 있다`,`Element Plus 를 국내에서 많이 쓰는 이유를 비교표로 설명할 수 있다`,`main.js 에 라이브러리와 CSS 를 등록하는 두 줄을 외우지 않고도 찾아 쓸 수 있다`,`태그로 쓰는 컴포넌트와 JS 함수로 부르는 컴포넌트(ElMessage 등)를 구분할 수 있다`,`el-form 검증과 직접 if 로 검증하는 방식의 차이를 안다`,`ElMessageBox.confirm 이 Promise 를 돌려준다는 점과 취소가 catch 로 간다는 점을 안다`],quiz:[{q:`el-button 은 나오는데 스타일이 하나도 없다. 무엇을 빠뜨렸나?`,a:`import 'element-plus/dist/index.css' 다. 컴포넌트 등록과 CSS 로드는 별개다.`},{q:`ElMessage 를 쓰려면 템플릿에 어떤 태그를 적어야 하는가?`,a:`적지 않는다. 태그가 아니라 JS 함수라서 import 해서 호출만 하면 화면에 알아서 떠오른다.`},{q:`ElMessageBox.confirm 에서 사용자가 취소를 눌렀다. 코드의 어디로 가는가?`,a:`catch 로 간다. 확인은 then, 취소는 catch 다. catch 가 없으면 처리되지 않은 Promise 경고가 뜬다.`},{q:`데이터가 오기 전 화면이 텅 비어 보인다. 어떤 컴포넌트가 어울리나?`,a:`el-skeleton 이다. 회색 유령 레이아웃을 미리 깔아 체감 속도를 높인다. 결과가 없을 때는 el-empty 를 쓴다.`}]}},Ul=e=>Hl[e]??null,Wl={class:`chapter-nav`},Gl={class:`nav-head`},Kl={key:0,class:`nav-title`},ql=[`title`,`aria-label`],Jl=[`title`,`onClick`],Yl={class:`chapter-no`},Xl={key:0,class:`chapter-info`},Zl={key:1,class:`chapter-meta`},Ql=[`title`],$l={key:0,class:`nav-legend`},eu={key:0},tu={id:`learning-content`,class:`learning-content`},nu={class:`chapter-head`},ru={class:`eyebrow`},iu={class:`chapter-summary`},au={class:`topic-chips`},ou={key:2,class:`challenge-stack`},su={key:3,class:`empty-state`},cu={key:4,class:`linked-assignments`},lu={class:`step`},uu=2,du=K({__name:`LearningView`,props:{chapterId:{type:String,default:``}},setup(e){te();let t=ee(),n=e,r=T(()=>{let e=Number(n.chapterId);return W.some(t=>t.id===e)?e:uu}),i=e=>{t.push({name:`learning`,params:{chapterId:String(e)}}),document.getElementById(`learning-content`)?.scrollIntoView({behavior:`smooth`,block:`start`})};u(()=>n.chapterId,e=>{e||t.replace({name:`learning`,params:{chapterId:String(uu)}})},{immediate:!0});let a=y(!1),o=T(()=>W.find(e=>e.id===r.value)),c=T(()=>Ul(r.value)),l=T(()=>ne(r.value)),d=T(()=>G(r.value)),f=T(()=>W.map(e=>{let t=ne(e.id),n=t.filter(e=>e.label.startsWith(`Code Challenge`));return{...e,total:n.length,live:t.filter(e=>e.practices.length>0).length,assignments:G(e.id).length}})),p=()=>{window.scrollTo({top:0,behavior:`smooth`})};return(e,t)=>{let n=m(`RouterLink`);return s(),k(C,null,[t[6]||=M(`section`,{class:`page-intro`},[M(`p`,{class:`eyebrow`},`LEARNING & CODE CHALLENGES`),M(`h1`,null,`학습 & 코드 챌린지`),M(`p`,null,[g(` 왼쪽에서 챕터를 고르면 해당 챕터의 `),M(`strong`,null,`코드 챌린지`),g(`가 열립니다. 각 챌린지는 `),M(`strong`,null,`관련 강의 내용 → 연습 항목 → 실습 화면`),g(` 순서로 이어집니다. `)])],-1),M(`section`,{class:h([`learning-layout`,{"nav-collapsed":a.value}])},[M(`aside`,Wl,[M(`div`,Gl,[a.value?x(``,!0):(s(),k(`p`,Kl,`학습 로드맵`)),M(`button`,{type:`button`,class:`nav-toggle`,title:a.value?`메뉴 펼치기`:`메뉴 접기`,"aria-label":a.value?`메뉴 펼치기`:`메뉴 접기`,onClick:t[0]||=e=>a.value=!a.value},A(a.value?`»`:`«`),9,ql)]),(s(!0),k(C,null,P(f.value,e=>(s(),k(`button`,{key:e.id,type:`button`,class:h([`chapter-item`,{active:e.id===r.value}]),title:a.value?`${e.title} · ${e.slides}`:``,onClick:t=>i(e.id)},[M(`span`,Yl,A(String(e.id).padStart(2,`0`)),1),a.value?x(``,!0):(s(),k(`span`,Xl,[M(`b`,null,A(e.title),1),M(`small`,null,A(e.slides),1)])),a.value?x(``,!0):(s(),k(`span`,Zl,[M(`i`,{class:h([`dot`,{live:e.live,none:!e.total}]),title:e.total?`코드 챌린지 ${e.total}개${e.live?` · 실습 화면 ${e.live}개`:``}`:`코드 챌린지 없음`},A(e.total||`–`),11,Ql)]))],10,Jl))),128)),a.value?x(``,!0):(s(),k(`p`,$l,[...t[1]||=[g(` 숫자 = 코드 챌린지 수 \xA0·\xA0 `,-1),M(`i`,{class:`dot live`},`n`,-1),g(` 실습 화면 있음 `,-1)]])),M(`button`,{type:`button`,class:`to-top`,title:`맨 위로`,"aria-label":`맨 위로 가기`,onClick:p},[t[2]||=M(`svg`,{viewBox:`0 0 24 24`,"aria-hidden":`true`},[M(`path`,{d:`m6 15 6-6 6 6`})],-1),a.value?x(``,!0):(s(),k(`span`,eu,`맨 위로`))])]),M(`div`,tu,[M(`header`,nu,[M(`div`,null,[M(`p`,ru,A(o.value.code)+` · 교안 `+A(o.value.slides),1),M(`h2`,null,A(o.value.title),1),M(`p`,iu,A(o.value.summary),1),M(`div`,au,[(s(!0),k(C,null,P(o.value.topics,e=>(s(),k(`span`,{key:e},A(e),1))),128))])])]),o.value.orientation?(s(),v(Vl,{key:0,orientation:o.value.orientation},null,8,[`orientation`])):x(``,!0),c.value?(s(),v(wl,{key:1,notes:c.value},null,8,[`notes`])):x(``,!0),l.value.length?(s(),k(`div`,ou,[(s(!0),k(C,null,P(l.value,e=>(s(),v(sl,{key:e.id,challenge:e},null,8,[`challenge`]))),128))])):(s(),k(`p`,su,[...t[3]||=[g(` 교안 기준으로 이 챕터에는 코드 챌린지가 없습니다. 아래 `,-1),M(`b`,null,`제출 과제`,-1),g(`로 바로 진행합니다. `,-1)]])),d.value.length?(s(),k(`section`,cu,[t[5]||=M(`p`,{class:`section-label`},`이 챕터의 제출 과제`,-1),(s(!0),k(C,null,P(d.value,e=>(s(),v(n,{key:e.id,class:`assignment-link`,to:`/assignments`},{default:b(()=>[M(`span`,lu,A(String(e.id).padStart(2,`0`)),1),M(`span`,null,[M(`b`,null,A(e.title),1),M(`small`,null,A(e.goal),1)]),t[4]||=M(`span`,{class:`arrow`},`→`,-1)]),_:2},1024))),128))])):x(``,!0)])],2)],64)}}},[[`__scopeId`,`data-v-94fb5490`]]);export{du as default};